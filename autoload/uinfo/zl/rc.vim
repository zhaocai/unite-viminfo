...
let s:UINFO_ZL_VERSION_CURRENT = 140

let s:VERSION_FACTOR = str2float('0.01')


function! uinfo#zl#rc#init() abort
    if exists('g:loaded_uinfo_zl')
        return
    endif
    ...
    if !exists("g:uinfo_zl_force_reload")
        let g:uinfo_zl_force_reload = 0
    endif

    if !exists("g:uinfo_zl_debug_mode")
        let g:uinfo_zl_debug_mode = 0
    endif

    let g:loaded_uinfo_zl = s:UINFO_ZL_VERSION_CURRENT * s:VERSION_FACTOR

endfunction



function! uinfo#zl#rc#load_guard(prefix, vim_version, uinfo_zl_version,exprs,...)
    "--------- ------------------------------------------------
    ...
    "   - uinfo_zl_version : minmium zl.vim version requirement.
    "                    set 0 to ignore.
    ...
    "
    " Example : >
    "   try | if !uinfo#zl#rc#load_guard(expand('<sfile>:t:r'), 703, 140, ['!&cp'])
    "           finish
    "       endif
    "   catch /^Vim\%((\a\+)\)\=:E117/
...
"   call Dfunc('uinfo#zl#rc#load_guard(' . a:prefix .' '. a:vim_version .' '. a:uinfo_zl_version .' '. join(a:exprs))
...
"    call Decho("loaded_var_name: " . l:loaded_var_name)
    ...
    if exists(l:loaded_var_name) && g:uinfo_zl_force_reload == 0
"       call Decho(l:loaded_var_name . ' loaded: return 0')
"       call Dret('uinfo#zl#rc#load_guard')
        return 0
    endif
    ...
    if a:vim_version > 0 && a:vim_version > v:version
        ...
        return 0
    elseif a:uinfo_zl_version > 0
        if !exists("g:loaded_uinfo_zl")
            echoerr 'zl.vim is required but not loaded'
            return 0
        endif

        if (a:uinfo_zl_version > s:UINFO_ZL_VERSION_CURRENT)
            echoerr l:loaded_var_name . ' requires zl library version '
                        \ . string(a:uinfo_zl_version * s:VERSION_FACTOR)
            return 0
        endif
    endif
    for expr in a:exprs
...
function! uinfo#zl#rc#script_force_reload(...)
    "--------- ------------------------------------------------
    " Desc    : Call to ignore uinfo#zl#rc#load_guard() and source.
    "
    ...
    let l:saved = g:uinfo_zl_force_reload
    let g:uinfo_zl_force_reload = 1

    exec "so " . script

    let g:uinfo_zl_force_reload = l:saved
endfunction




function! uinfo#zl#rc#init_python()
    if has("python3")
        let g:uinfo_zl_py = 'py3'
        let g:uinfo_zl_pyeval = 'py3eval'
    elseif has("python")
        let g:uinfo_zl_py = 'py'
        let g:uinfo_zl_pyeval = 'pyeval'
    else 
        let g:uinfo_zl_py = ''
        let g:uinfo_zl_pyeval = ''
    endif
endfunction
...
" ============================================================================
function! uinfo#zl#rc#set_default(var, ...)
    "--------- ------------------------------------------------
...
"
" Note: 
"   add this call to other files if they need to check g:loaded_uinfo_zl
" or g:uinfo_zl_install_path
"
call uinfo#zl#rc#init()
